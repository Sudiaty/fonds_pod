...
use crate::infrastructure::persistence::models::File;
use std::error::Error;
use std::path::Path;
use chrono::Local;
...
impl FileService {
    /// Generate the next file number for a given series
    /// Format: [SeriesNo]-[2-digit sequence]
    pub fn generate_file_no(
        db_path: &Path,
        series_no: &str,
    ) -> Result<String, Box<dyn Error>> {
        let mut conn = establish_connection(db_path)?;
        let seq = queries::get_next_sequence(&mut conn, series_no)?;
        let formatted_seq = queries::format_sequence(seq, 2);
        Ok(format!("{}-{}", series_no, formatted_seq))
    }
    ...
    /// List all files for a series
    pub fn list_files_by_series(db_path: &Path, series_no: &str) -> Result<Vec<File>, Box<dyn Error>> {
        let mut conn = establish_connection(db_path)?;
        queries::list_files_by_series(&mut conn, series_no)
    }

    /// Get a single file by number
    pub fn get_file(db_path: &Path, file_no: &str) -> Result<Option<File>, Box<dyn Error>> {
        let mut conn = establish_connection(db_path)?;
        queries::get_file(&mut conn, file_no)
    }
