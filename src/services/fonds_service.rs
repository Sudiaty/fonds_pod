...
impl FondsService {
    /// Generate the next fonds number for a given classification code
    /// Format: [ClassificationCode][2-digit sequence]
    pub fn generate_fond_no(
        db_path: &Path,
        classification_code: &str,
    ) -> Result<String, Box<dyn Error>> {
        let mut conn = establish_connection(db_path)?;
        let seq = queries::get_next_sequence(&mut conn, classification_code)?;
        let formatted_seq = queries::format_sequence(seq, 2);
        Ok(format!("{}{}", classification_code, formatted_seq))
    }
    ...
    /// Get a single fond by number
    pub fn get_fond(db_path: &Path, fond_no: &str) -> Result<Option<Fond>, Box<dyn Error>> {
        let mut conn = establish_connection(db_path)?;
        queries::get_fond(&mut conn, fond_no)
    }
    
    /// Delete a fond and all associated data
    pub fn delete_fond(db_path: &Path, fond_no: &str) -> Result<bool, Box<dyn Error>> {
        let mut conn = establish_connection(db_path)?;
        // Note: Series and files should be handled by foreign key cascades
        // or explicitly deleted here if needed
        queries::delete_fond_schemas(&mut conn, fond_no)?;
        queries::delete_fond(&mut conn, fond_no)
    }
    ...
    /// Update series for a fond (regenerate without deleting existing)
    /// This is used when the user wants to refresh series after schema changes
    pub fn update_series_for_fond(
        db_path: &Path,
        fond_no: &str,
    ) -> Result<usize, Box<dyn Error>> {
        // Get the fond to get created_at
        let fond = Self::get_fond(db_path, fond_no)?
            .ok_or_else(|| format!("Fond '{}' not found", fond_no))?;
        
        // Generate new series (will skip existing ones)
        Self::generate_series_for_fond(db_path, fond_no, &fond.created_at)
    }
