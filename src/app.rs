...
use fonds_pod_lib::viewmodels::{SettingsViewModel, AboutViewModel, FondClassificationViewModel, HomeViewModel};
...
impl App {
    pub fn initialize(ui_handle: &AppWindow) -> Self {
        ...
        let fond_classification_vm = Rc::new(RefCell::new(
            FondClassificationViewModel::new(Rc::clone(&settings_service))
        ));
    ...
    pub fn setup_ui_callbacks(&self, ui_handle: &AppWindow) {
        ...
        FondClassificationViewModel::setup_callbacks(Rc::clone(&self.fond_classification_vm), ui_handle);
    ...
    fn setup_common_callbacks(&self, ui_handle: &AppWindow) {
        ...
        ui_handle.on_page_changed({
            ...
            move |page_name| {
                ...
                if page_name == "classification" {
                    if let Some(ui) = ui_weak.upgrade() {
                        fond_classification_vm.borrow_mut().on_page_enter(&ui);
                    }
                }
            }
        });
